...
from app.models import ClientPC, License, Cafe
...
def pc_heartbeat(
    ...
    db.commit()
    try:
        log_action(db, None, 'pc_heartbeat', f'PC:{pc_id} status:{pc.status}', request.client.host if request and request.client else None)
    except Exception:
        pass
