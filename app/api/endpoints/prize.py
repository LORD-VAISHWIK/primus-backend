...
from app.models import Prize, PrizeRedemption, User, CoinTransaction
...
def create_prize(prize: PrizeIn, current_user=Depends(require_role("admin")), db: Session = Depends(get_db)):
...
def redeem_prize(prize_id: int, current_user=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    r = PrizeRedemption(user_id=user.id, prize_id=prize.id, timestamp=datetime.utcnow(), status="pending")
    db.add(r)
    db.commit()
    db.refresh(r)
    return r
