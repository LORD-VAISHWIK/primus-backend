...
def create_group(group: UserGroupIn, current_user=Depends(require_role("admin")), db: Session = Depends(get_db)):
...
def assign_user_group(user_id: int, group_id: int, current_user=Depends(require_role("admin")), db: Session = Depends(get_db)):
