...
CONTENT = [
    ...
    ("", False),
    ("Overview", True),
    ("Lance/Primus is an end-to-end cafe and PC-lab management platform combining a Python FastAPI backend, React admin frontends, and a Windows C# WPF client to control PCs.", False),
    ("", False),
    ("Core Components", True),
    ("- Backend API (Python/FastAPI): central service exposing REST and WebSockets; SQLite via SQLAlchemy.", False),
    ("- Admin Web UIs (React/Vite): `admin-app` and `Primus/primus-client` for staff/admin operations.", False),
    ("- Windows Client (Primus-client.exe): C# .NET 8 WPF app that binds to a PC, receives commands, and enforces locks.", False),
    ("", False),
    ...
    ("- Firebase Admin SDK is supported for token verification but email/password JWT is primary.", False),
    ("", False),
    ("PC Management & Sessions", True),
    ("- Bind/register client PCs to cafes and groups; track current_user on each PC.", False),
    ("- Start/stop sessions; compute time remaining from wallet, offers, and pricing rules.", False),
    ("- Auto time-left broadcast every minute and auto-lock at zero minutes (background task).", False),
    ...
    ("- Wallet: maintain wallet_balance and coins_balance; convert to time via active pricing rules.", False),
    ("- Offers & Coupons: grant hours/discounts; user offers are consumed as time.", False),
    ("- Payments & Billing: record payments and invoices; integrate with external providers via webhooks.", False),
    ("", False),
    ...
    ("- Built with .NET 8 WPF; connects to /ws/pc/{pc_id}; enforces lock/unlock and updates.", False),
...
def generate_pdf(output_path: str):
    ...
    for text, is_heading in CONTENT:
        if y < 1.0 * inch:
            ...
            y = height - top_margin
        if text:
            y = draw_wrapped_text(c, text, left_margin, y, max_width, bold=is_heading, leading=16 if is_heading else 14)
        else:
            y -= 6
